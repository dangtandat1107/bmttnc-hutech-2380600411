...
def encode_image(image_path, message):
    ...
    # Chuyển đổi thông điệp văn bản sang chuỗi bit nhị phân (mỗi ký tự 8-bit)
    binary_message = ''.join(format(ord(char), '08b') for char in message)
    # Đánh dấu chuỗi bit kết thúc thông điệp để phía decrypt biết đường dừng lại
    binary_message += '1111111111111110' 
    
    data_index = 0
    for row in range(height):
        for col in range(width):
            # Lấy danh sách giá trị màu RGB của pixel hiện tại
            ...
            # Cập nhật lại pixel đã bị giấu tin vào ảnh
